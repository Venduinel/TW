...
class UserController implements Controller {
    ...
    if (!emailOrName) {
        return response.status(400).json({ error: 'Email or username is required' });
    }
    ...
    try {
        ...
        await emailService.sendResetPasswordEmail(user.email, newPassword);

        response.status(200).json({ message: 'New password has been sent to your email' });
    } catch (error) {
