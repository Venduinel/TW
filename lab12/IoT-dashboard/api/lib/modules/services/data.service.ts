...
export default class DataService {
      ...
      await Promise.all(
        Array.from({ length: supportedDevicesNum }, async (_, i) => {
          try {
            ...
            console.error(
              `Błąd podczas pobierania danych dla urządzenia ${i}: ${error.message}`
            );
            latestData.push({ deviceId: i } as IData);
          }
        })
  ...
  public async deleteDataInRange(deviceId: number, from: Date, to: Date) {
    try {
  ...
  public async getLastHourDataByDeviceId(
    deviceId: number,
    minutes: number = 60
  ) {
